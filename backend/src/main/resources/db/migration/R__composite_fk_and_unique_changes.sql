...
SET @idx_exists := (
  ...
  WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'seasons' AND INDEX_NAME = 'idx_seasons_league_id_pk'
);
...
SET @fk_old_exists := (
  ...
  WHERE CONSTRAINT_SCHEMA = @db AND CONSTRAINT_NAME = 'fk_match_season' AND TABLE_NAME = 'matches'
);
...
SET @fk_new_exists := (
  SELECT COUNT(*) FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS
  WHERE CONSTRAINT_SCHEMA = @db AND CONSTRAINT_NAME = 'fk_match_league_season' AND TABLE_NAME = 'matches'
);
...
SET @uk_old_exists := (
  SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'matches' AND INDEX_NAME = 'uk_match_league_round_home_away'
);
...
SET @uk_new_exists := (
  ...
  WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'matches' AND CONSTRAINT_NAME = 'uk_match_league_season_date_home_away'
);
...
SET @idx2_exists := (
  SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'matches' AND INDEX_NAME = 'idx_matches_league_season'
);
