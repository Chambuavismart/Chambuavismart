...
public class FixtureUploadService {
    ...
    public UploadResultDTO uploadCsv(Long leagueId, String season, boolean fullReplace, String csvText) {
        ...
        // Deduplicate within batch to avoid DB unique constraint conflicts
        fixtures = deduplicateAndMerge(fixtures);
        int processed = 0;
    ...
    public UploadResultDTO upload(FixturesUploadRequest req){
        ...
        if (req.isFullReplace()) {
            // In full replace, deduplicate/merge in-memory before bulk insert to avoid unique key collisions
            fixtures = deduplicateAndMerge(fixtures);
    ...
    private void validateAndNormalize(List<Fixture> fixtures, League league, String season, List<String> errors) {
        ...
        for (int i = 0; i < fixtures.size(); i++) {
            Fixture f = fixtures.get(i);
            String homeKey = (f.getHomeTeam()==null?"":f.getHomeTeam().trim().toLowerCase().replaceAll("\\s+"," "));
            String awayKey = (f.getAwayTeam()==null?"":f.getAwayTeam().trim().toLowerCase().replaceAll("\\s+"," "));
            String key = league.getId() + "|" + f.getDateTime() + "|" + homeKey + "|" + awayKey;
            if (!seen.add(key)) {
    ...
    // Collapse duplicate fixtures within the same batch by unique key and merge best data
    private List<Fixture> deduplicateAndMerge(List<Fixture> fixtures) {
        if (fixtures == null || fixtures.isEmpty()) return fixtures;
        java.util.Map<String, Fixture> map = new java.util.LinkedHashMap<>();
        for (Fixture f : fixtures) {
            String homeKey = (f.getHomeTeam()==null?"":f.getHomeTeam().trim().toLowerCase().replaceAll("\\s+"," "));
            String awayKey = (f.getAwayTeam()==null?"":f.getAwayTeam().trim().toLowerCase().replaceAll("\\s+"," "));
            Long leagueId = (f.getLeague() != null && f.getLeague().getId() != null) ? f.getLeague().getId() : 0L;
            String key = leagueId + "|" + f.getDateTime() + "|" + homeKey + "|" + awayKey;
            Fixture existing = map.get(key);
            if (existing == null) {
                map.put(key, f);
            } else {
                map.put(key, mergeFixtures(existing, f));
            }
        }
        return new java.util.ArrayList<>(map.values());
    }

    private Fixture mergeFixtures(Fixture a, Fixture b) {
        if (a == null) return b;
        if (b == null) return a;
        boolean aFinished = a.getStatus() == FixtureStatus.FINISHED;
        boolean bFinished = b.getStatus() == FixtureStatus.FINISHED;
        Fixture primary = a;
        Fixture secondary = b;
        if (aFinished && !bFinished) {
            ...
            // if both same status, prefer the one with more score data
            int aScoreKnown = (a.getHomeScore() != null ? 1 : 0) + (a.getAwayScore() != null ? 1 : 0);
            int bScoreKnown = (b.getHomeScore() != null ? 1 : 0) + (b.getAwayScore() != null ? 1 : 0);
            if (bScoreKnown > aScoreKnown) { primary = b; secondary = a; }
        }
        // Merge missing properties from secondary
        if (primary.getHomeScore() == null && secondary.getHomeScore() != null) primary.setHomeScore(secondary.getHomeScore());
        if (primary.getAwayScore() == null && secondary.getAwayScore() != null) primary.setAwayScore(secondary.getAwayScore());
        if ((primary.getRound() == null || primary.getRound().isBlank()) && secondary.getRound() != null) primary.setRound(secondary.getRound());
        if (primary.getStatus() == null || (primary.getStatus() == FixtureStatus.UPCOMING && secondary.getStatus() == FixtureStatus.FINISHED)) {
            primary.setStatus(secondary.getStatus());
        }
        return primary;
    }
}
