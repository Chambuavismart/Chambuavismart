...
2
        ...
        <div class="hint" style="margin: -4px 0 8px 0;">
          ...
          Tip: You can also mark abandoned matches using ABN/ABD/ABANDONED in the score slot or as a status line after the date; the system will store them as Abandoned and automatically update later when an official result is uploaded.
        </div>
...
-
      ...
      <div *ngIf="success && (insertedCount !== null && deletedCount !== null)" class="card" style="border-color:#3b82f6">
        <h3 style="color:#1e40af">Upload Summary</h3>
        <div>Inserted: {{insertedCount}}</div>
        <div>Deleted (full replace): {{deletedCount}}</div>
        <div *ngIf="processedMatches">Processed: {{processedMatches}}</div>
      </div>
      ...
      <div *ngIf="results.updated?.length" class="card" style="border-color:#10b981">
        ...
        <table style="width:100%">
          ...
          <tr *ngFor="let u of results.updated">
            <td>{{u.fixtureId}}</td>
            <td>
              {{cleanTeam(u.homeTeam)}}
              <span *ngIf="isAbnName(u.homeTeam) || isAbnName(u.awayTeam)" class="badge badge-abn" title="Abandoned as uploaded; will update when official result appears">Abandoned</span>
            </td>
            <td>{{cleanTeam(u.awayTeam)}}</td>
            <td>{{u.result}}</td>
            <td>
              <span class="badge" [ngClass]="statusClass(u.status)">{{u.status}}</span>
            </td>
          </tr>
        </table>
      </div>
      <div *ngIf="(warnObjects?.length || 0) > 0 || (warnStrings?.length || 0) > 0" class="card" style="border-color:#fb923c">
        ...
        <ul *ngIf="(warnObjects?.length || 0) > 0">
          <li *ngFor="let w of warnObjects">
            {{cleanTeam(w.homeTeam)}} vs {{cleanTeam(w.awayTeam)}} — {{w.reason}}
            <span *ngIf="isAbnName(w.homeTeam) || isAbnName(w.awayTeam)" class="badge badge-abn">Abandoned</span>
          </li>
        </ul>
      ...
      <div *ngIf="results.skipped?.length" class="card" style="border-color:#ef4444">
        ...
        <ul>
          <li *ngFor="let s of results.skipped">
            {{cleanTeam(s.homeTeam)}} vs {{cleanTeam(s.awayTeam)}} — {{s.reason}}
            <span *ngIf="isAbnName(s.homeTeam) || isAbnName(s.awayTeam)" class="badge badge-abn">Abandoned</span>
          </li>
        </ul>
      </div>
  ...
  styles: [`
    ...
    .badge{display:inline-block;padding:2px 6px;border-radius:999px;font-size:11px;line-height:1;border:1px solid transparent;margin-left:6px}
    .badge-played{background:#ecfdf5;color:#065f46;border-color:#10b981}
    .badge-scheduled{background:#eff6ff;color:#1e3a8a;border-color:#60a5fa}
    .badge-postponed{background:#fff7ed;color:#9a3412;border-color:#fdba74}
    .badge-awarded{background:#f0fdf4;color:#14532d;border-color:#86efac}
    .badge-abn{background:#fef2f2;color:#991b1b;border-color:#ef4444}
  `]
})
export class MatchUploadComponent {
  ...
  // UI helpers for status and abandoned markers
  isAbnName(name: any): boolean {
    const s = (name || '').toString();
    return /\[ABN\]/i.test(s) || /\(ABN\)/i.test(s) || /\(ABANDONED\)/i.test(s) || /\bABN\b/i.test(s) || /\bABANDONED\b/i.test(s);
  }
  cleanTeam(name: any): string {
    const s = (name || '').toString();
    return s
      .replace(/\[ABN\]\s*/ig, '')
      .replace(/\s*\(ABN\)\s*/ig, ' ')
      .replace(/\s*\(ABANDONED\)\s*/ig, ' ')
      .replace(/\bABN\b/ig, ' ')
      .replace(/\bABANDONED\b/ig, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  }
  statusClass(status: any): string {
    const s = (status || '').toString().toUpperCase();
    if (s === 'PLAYED') return 'badge-played';
    if (s === 'SCHEDULED') return 'badge-scheduled';
    if (s === 'POSTPONED') return 'badge-postponed';
    if (s === 'AWARDED') return 'badge-awarded';
    if (s === 'ABANDONED') return 'badge-abn';
    return '';
  }
  ...
  private resetFeedback(){ this.message=''; this.success=false; this.errors=[]; this.processedMatches = 0; this.ignoredLines = []; this.warnStrings = []; this.warnObjects = []; this.insertedCount = 0; this.deletedCount = 0; }
