    ...
    <div class="container">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <div style="display:flex; align-items:center; gap:8px; flex-wrap: wrap;">
            <button class="btn" type="button" (click)="downloadAllOnPage()" [disabled]="loading || items.length === 0">Download all on page</button>
            <button class="btn" type="button" (click)="downloadSelected()" [disabled]="loading || selectedIds.size === 0">Download selected</button>
...
export class AnalysisPdfsComponent implements OnInit {
  ...
  load(){
    ...
    this.http.get<any>(this.api('/api/matches/analysis-pdfs?page=0&size=50')).subscribe({
      next: res => { this.items = Array.isArray(res?.content) ? res.content : []; this.loading = false; this.selectedIds.clear(); },
  ...
  allSelected(): boolean { return this.items.length > 0 && this.items.every(it => this.selectedIds.has(it.id)); }
  toggleOne(id: number, ev: any){ if (ev?.target?.checked) this.selectedIds.add(id); else this.selectedIds.delete(id); }
  toggleAll(ev: any){
    const on = !!ev?.target?.checked; this.selectedIds.clear();
    if (on) { for (const it of this.items) { if (it?.id != null) this.selectedIds.add(it.id); } }
  }
  ...
  async downloadSelected(){
    ...
    await this.mergeAndDownload(ids);
  }
  ...
  private mergeAndDownload(ids: number[]){
    this.merging = true; this.error = '';
    const url = this.api('/api/matches/analysis-pdfs/merge');
    return new Promise<void>((resolve) => {
      this.http.post(url, { ids }, { responseType: 'blob' as 'json' }).subscribe({
        next: (blob: any) => { this.merging = false; this.triggerDownload(blob as Blob); resolve(); },
        error: err => { this.merging = false; this.error = (err?.error?.text) || 'Failed to merge and download PDFs.'; resolve(); }
      });
    });
  }
  private triggerDownload(blob: Blob){
    ...
    const name = `Fixture_Analysis_Selected_${ts.getFullYear()}-${pad(ts.getMonth()+1)}-${pad(ts.getDate())}_${pad(ts.getHours())}.${pad(ts.getMinutes())}.pdf`;
